...
class DeepQDriver(object):
    ...
    def __init__(self, checkpoint_path="deep_q_xiaocar_networks", playback_mode=False, verbose_logging=True):
        """
        Example of deep q network for robot driver
        ...
        self._input_layer, self._output_layer = DeepQDriver._create_network()

        self._action = tf.placeholder("float", [None, self.ACTIONS_COUNT])
        self._target = tf.placeholder("float", [None])

        readout_action = tf.reduce_sum(tf.multiply(self._output_layer, self._action), axis=1)
    ...
    def get_keys_pressed(self, screen_array, reward, terminal):
        ...
        if self._last_state is None:
            ...
            return DeepQDriver._key_presses_from_action(self._last_action)
    ...
    def _create_network():
        # network weights
        convolution_weights_1 = tf.Variable(tf.truncated_normal([8, 8, DeepQDriver.STATE_FRAMES, 32], stddev=0.01))
        ...
        feed_forward_weights_2 = tf.Variable(tf.truncated_normal([256, DeepQDriver.ACTIONS_COUNT], stddev=0.01))
        feed_forward_bias_2 = tf.Variable(tf.constant(0.01, shape=[DeepQDriver.ACTIONS_COUNT]))

        input_layer = tf.placeholder("float", [None, DeepQDriver.RESIZED_SCREEN_X, DeepQDriver.RESIZED_SCREEN_Y,
                                               DeepQDriver.STATE_FRAMES])
